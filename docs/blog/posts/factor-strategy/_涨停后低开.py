...
def buy(context):
    ...
    if len(hl_list) != 0:
        ...
        df["open_pct"] = [
            current_data[s].day_open / df.loc[s, "close"] for s in stock_list
        ]
        df = df[
            (0.96 <= df["open_pct"]) & (df["open_pct"] <= 0.97)
        ]  # 低开越多风险越大，选择3个多点即可
...
def get_hl_count_df(hl_list, date, watch_days):
    ...
    df = pd.DataFrame(
        index=hl_list,
        data={"count": hl_count_list, "extreme_count": extreme_hl_count_list},
    )
    return df
