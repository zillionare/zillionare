...
tags: "[大模型, 中证1000, AI量化, 回测, 量化交易, Agent]"
excerpt: "9个月前LLM交易实验夏普仅0.019完败，如今AI飞速发展9个月后卷土重来。本文在中证1000上设计三个递进实验：从仅预测仓位、到引入状态统计与风险调整，逐步挖掘LLM交易决策背后的原理与改进路径。用真实回测数据检验Agent是否具备稳定盈利潜力，量化投资者不容错过。"
---
