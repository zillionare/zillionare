...
现在，我们开始回测，并使用quantstats生成标准化的投资组合分析报表。同时创建等权基准进行对比。
...
# 首先创建等权基准
def create_equal_weight_benchmark(test_data, initial_amount=100000):
    """
    创建等权基准组合
    """
    print("📊 创建等权基准...")
    ...
    test_assets = sorted(test_data.index.get_level_values('asset').unique())

    benchmark_values = [initial_amount]
    benchmark_dates = [test_dates[0]]
    ...
    for i in range(1, len(test_dates)):
        ...
        # 获取当前日期和前一日期的价格数据
        current_data = test_data.loc[current_date]
        prev_data = test_data.loc[prev_date]

        # 计算每只股票的收益率
        daily_returns = []
        for asset in test_assets:
            if asset in current_data.index and asset in prev_data.index:
                current_price = current_data.loc[asset, 'close']
                prev_price = prev_data.loc[asset, 'close']
                if prev_price > 0:
        ...
        # 计算等权平均收益率
        if daily_returns:
            equal_weight_return = np.mean(daily_returns)
            new_value = benchmark_values[-1] * (1 + equal_weight_return)
    ...
    return benchmark_dates, benchmark_values

# 创建基准
benchmark_dates, benchmark_values = create_equal_weight_benchmark(test_data)
...
benchmark_df = pd.DataFrame({
    'date': benchmark_dates[:len(benchmark_values)],
...
# 等权基准绩效
print("\n📊 等权基准绩效:")
print(f"总收益率: {qs.stats.comp(benchmark_returns):.2%}")
print(f"年化收益率: {qs.stats.cagr(benchmark_returns):.2%}")
print(f"年化波动率: {qs.stats.volatility(benchmark_returns):.2%}")
print(f"夏普比率: {qs.stats.sharpe(benchmark_returns):.3f}")
print(f"最大回撤: {qs.stats.max_drawdown(benchmark_returns):.2%}")
print(f"卡尔马比率: {qs.stats.calmar(benchmark_returns):.3f}")
print(f"胜率: {qs.stats.win_rate(benchmark_returns):.2%}")

# 相对绩效分析
print("\n📈 相对绩效分析:")
excess_returns = portfolio_returns - benchmark_returns
print(f"超额收益率: {qs.stats.comp(excess_returns):.2%}")
print(f"年化超额收益: {qs.stats.cagr(excess_returns):.2%}")
print(f"信息比率: {qs.stats.sharpe(excess_returns):.3f}")
print(f"跟踪误差: {qs.stats.volatility(excess_returns):.2%}")

# 胜率分析
win_days = (excess_returns > 0).sum()
total_days = len(excess_returns)
win_rate = win_days / total_days
print(f"相对基准胜率: {win_rate:.2%} ({win_days}/{total_days})")
...
plt.subplot(2, 1, 1)
plt.plot(portfolio_cumulative.index, portfolio_cumulative.values, label='强化学习模型', linewidth=2)
plt.plot(benchmark_cumulative.index, benchmark_cumulative.values, label='等权基准', linewidth=2)
plt.title('累计收益对比')
plt.ylabel('累计收益')
plt.legend()
...
plt.plot(excess_cumulative.index, excess_cumulative.values, label='超额收益', color='green', linewidth=2)
...
print(f"\n📊 最终投资组合价值对比:")
print(f"强化学习模型: ¥{portfolio_values[-1]:,.2f}")
print(f"等权基准: ¥{benchmark_values[-1]:,.2f}")
print(f"超额收益: ¥{portfolio_values[-1] - benchmark_values[-1]:,.2f}")
```

这个示例演示了一个完整的强化学习交易模型，不过，它没有经过参数调优，因子也没有精心选择，比起它的表现，我们更关心代码的完整性。

总而言之，强化学习为量化交易打开了一扇通往更高维度智能的大门。它不再是让机器模仿人类，而是让机器在模拟的市场中自我进化、自我博弈，最终习得超越人类直觉的交易智慧。这条路充满挑战，但也同样充满机遇。

那么，你准备好，让你的第一个交易Agent，开始它的"进化之旅"了吗？
