...
# 添加startup.py路径
sys.path.insert(
    0, os.path.expanduser("~/workspace/cheese_course/docs/factor-analysis/assets")
)
...
def calculate_williams_r_components(bars: pd.DataFrame) -> Tuple[pd.Series, pd.Series]:
    ...
    if not np.allclose(sum_check[mask], 1.0, atol=1e-6):
        warnings.warn(
            "Williams R components do not sum to 1, possible calculation error"
        )

    return wr_up, wr_down


def calculate_ubl_factor_improved(
    bars: pd.DataFrame,
    ...
    rolling_upper_shadow = upper_shadow_ratio.rolling(
        window=lookback_period, min_periods=min_periods
    ).mean()
    ...
    if standardize:
        ...
        rolling_mean = ubl_factor.rolling(
            window=lookback_period * 2, min_periods=lookback_period
        ).mean()
...
def compare_implementations(
    bars: pd.DataFrame, lookback_period: int = 20
) -> pd.DataFrame:
    """
    比较原始实现和改进实现的差异
    """
    ...
    improved_factor = calculate_ubl_factor_improved(
        bars, lookback_period, standardize=False
    )
...
def main():
    ...
    high_prices = np.maximum(open_prices, close_prices) * (
        1 + np.random.exponential(0.01, 100)
    )
    ...
    # 计算改进版UBL因子
    ubl_factor = calculate_ubl_factor_improved(
        bars, lookback_period=20, standardize=True
    )
