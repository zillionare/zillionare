...
TCN 的解决方案很简单：**只在序列的过去方向（左侧）填充数据**。

但在 PyTorch 等框架中，`nn.Conv1d` 的 `padding` 参数默认是在左右两侧对称填充的。因此在具体实现时，我们通常先在两侧填充，然后**显式地裁剪掉右侧多余的输出**，以严格保证"因果性"。
...
class TCNBlock(nn.Module):
    ...
    def forward(self, x):
        ...
        # 第一层卷积及因果裁剪（修正点：必须裁剪掉右侧多余的 padding，否则会泄露未来信息并导致维度错误）
        out = self.conv1(x)
...
class TCN(nn.Module):
    ...
    def forward(self, x):
        ...
        out = out[:, :, -1]  # 取最后一个时间步，此时形状为 [batch, num_channels]
        return self.fc(out)  # 修正点：out 已经是 2D 张量，直接传入 fc 即可，无需 transpose
```
...
在长序列训练中，LSTM 容易遇到梯度消失或爆炸问题，即便有门控机制也难以彻底解决。而 TCN 借助于**残差连接**和**因果卷积的结构**，反向传播路径更短（层数呈对数级增长），梯度能够更稳定地传递，模型训练更不容易崩溃，收敛过程更平滑。
...
虽然 TCN 训练很快，但在**增量推理**（如流式数据的实时预测）时存在劣势。LSTM 只需要保存上一步的隐藏状态即可预测下一步；而 TCN 为了预测最新的一步，需要将长度等于"感受野"的历史数据重新输入网络计算（除非使用复杂的缓存队列进行工程优化），这在部署时会带来更大的内存和计算负担。

*(注：许多人误以为 TCN 不能处理变长序列，这是个常见误区。实际上作为全卷积网络，TCN 天然支持任意长度的输入，只是在单步推理时对历史窗口的数据依赖更重。)*
