...
from helper import get_calendar, load_bars_tushare_all, pro_api, resample_to_month
...
class Moonshot:
    ...
    def load_dividend_yield(self) -> None:
        ...
        # 确保date列是datetime类型
        if not pd.api.types.is_datetime64_any_dtype(dividend_data['date']):
    ...
    def dividend_yield_screen(self, n: int = 500)->pd.Series:
        """股息率筛选方法
        ...
        if 'dividend_yield' not in self.data.columns:
            raise ValueError("数据中不存在dividend_yield列，无法应用筛选器")
        ...
        # 按date分组，对dividend_yield进行排名筛选
        dividend_flags = self.data.groupby(level='month')['dividend_yield'].transform(rank_top_n)
...
if __name__ == '__main__':
    ...
    # daily_bars = load_bars_tushare_all(start, end)
    daily_bars = pd.read_parquet("/tmp/bars.parquet").reset_index()
    moonshot = Moonshot(daily_bars)

    # 添加股息率筛选器
    moonshot.screen(moonshot.dividend_yield_screen, n=500).calculate_returns()

    report_file = Path(__file__).parent / "report.html"
    moonshot.analyzer.generate_html_report(output=report_file)
