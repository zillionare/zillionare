...
### 安装到Linux操作系统
您可以在这里下载[大富翁安装程序](_attachment/zillionare.sh)，并在命令行界面运行以下命令：
```bash
# 下载安装程序到本地
curl http://www.jieyu.ai/_attachment/zillionare.sh -o zillionare.sh
...
首先，从[这里](http://www.jieyu.ai/_attachment/zillionare.tar.gz)下载压缩包。然后将其解压到某个**空**文件夹。
