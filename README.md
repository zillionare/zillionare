...
作者从2019年起开始关注量化交易。量化交易的基础是必须要有可靠和实时的数据。在试用了一些商业和开源的产品后，作者决定自己来开发一个产品，主要是要解决获取实时、低成本数据的问题，也有一些品控和技术风格选择上的偏好考虑。
...
# **如何开始大富翁财富之旅？**
    想立即开始？您可以从[新手入门](getting_started.md)开始，这个教程将带领您安装大富翁，完成设置和初始化任务。

    [大富翁量化教程](tutorial.md)将带领您学习大富翁提供的各种功能，以及如何编写策略。在教程中，作为示例，我们也会分享自己的一些策略，从量化因子到人工智能的都有。

    [大富翁量化框架深度解析](developer_guide.md)从大富翁开发者的角度，介绍了大富翁开发过程中的技术选型思考、各种技术栈包括构建工具链如何使用。这也是一本面向工程实践的Python学习手册。
