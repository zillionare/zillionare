...
大富翁是一个开源项目，欢迎大家加入，以各种方式参与！

## 协同开发者

大富翁使用State of the Art的技术进行开发，在开发标准上遵循社区最佳实践，比如我们使用了Poetry进行项目管理，使用Mkdocs撰写文档，使用Github CI/CD来进行持续集成/发布，单元测试覆盖率保持在90%以上，等等。参与本项目开发，不仅可以学习到量化交易软件开发知识，还有助于您养成良好的软件工程能力。

当前可以供开发的功能（模块）有：

1. 接入更多上游数据源（当前仅接入了聚宽）
2. 性能提升重构。大富翁框架提供了非常领先的性能，但仍有进一步拓展的空间，包括：
    a. 优化行情数据在redis中的存储方式，以减少20%左右的内存占用。
    b. 设计支持分钟级\tick级数据的存储结构
    c. 数据校验机制

## 分享量化因子和交易策略
您可以把自己的量化因子和交易策略分享出来，跟大家一起探讨。

## 参与编写文档和教程
大富翁提供了较完善的文档和教程，但完全从用户的角度来写，可能会有完全不同的视角。

## 支持我们的开发
如果您希望支持我们的开发，您可以为本项目点赞，谢谢！
