...
def after_code_changed(context):
    ...
    run_daily(
        trade_afternoon, time="14:00", reference_security="399101.XSHE"
    )  # 下午交易检查
...
def get_stock_list(context):
    ...
    # 过滤最近卖出的股票
    recently_sold_stocks = [
        stock for date in g.sold_stock_record for stock in g.sold_stock_record[date]
    ]
    ...
    if final_list:
        ...
        final_list = [
            ...
            if stock in g.hold_list or last_prices[stock][-1] <= g.highest
        ]
...
def filter_audit(context, code):
    ...
    q = query(
        finance.STK_AUDIT_OPINION.code, finance.STK_AUDIT_OPINION.report_type
    ).filter(
...
def my_trade(context):
    ...
    if g.trading_signal == False:
        ...
        if hour == 9 and minute == 30:
            ...
            target_etf_rank = get_rank(
                context, g.stock_pool
            )  # Returns ['ETF_CODE', score]
...
def weekly_adjustment(context):
    ...
    if g.trading_signal:
        ...
        buy_security(
            context, buy_list, g.stock_num - len(hold_list)
        )  # Adjust number to buy based on target and current holds
...
def buy_security(context, target_list, num_to_buy):
    ...
    if target_buy_count > 0 and context.portfolio.available_cash > 0:
        ...
        for stock in target_list:
            ...
            if (
                context.portfolio.available_cash < 1000
            ):  # Assuming a minimum value needed
...
def record_recently_sold_stocks(context, stock):
    ...
    if stock not in g.stock_pool:
        ...
        trade_days = get_trade_days(
            start_date=current_date - datetime.timedelta(days=g.record_days),
...
def hyxx(stocks):
    ...
    return pd.Series(
        {
            stock: (
                industry_data[stock]["sw_l1"]["industry_name"]
...
def get_rank(context, stock_pool):
    ...
    for etf_code in g.stock_pool:
        ...
        try:
            data = attribute_history(
                etf_code, g.momentum_day, "1d", ["close"], df=True, skip_paused=True
            )
            if (
                data is None or data.empty or len(data) < 2
            ...
            score = np.polyfit(np.arange(len(normalized_prices)), normalized_prices, 1)[
                0
            ]
    ...
    if top_etf_code in etf_data_log:
        ...
        log.info(
            f"其最近 {g.momentum_day} 天收盘价数据:\n{etf_data_log[top_etf_code].tail(3)}"
        )
    else:
        log.info(
            f"最佳动量 ETF: {top_etf_code} (Score: {rank[0][1]:.4f}) (无详细数据记录)"
        )
...
def check_remain_amount(context):
    if g.trading_signal:
        ...
        if len(g.hold_list) < g.stock_num:
            ...
            log.info(
                "有余额可用"
                + str(round((context.portfolio.cash), 2))
                + "元。买入"
                + str(target_list)
            )
...
def calculate_angle_signal(context, stock_code="399101.XSHE"):
    ...
    if (
        ...
        log.info(
            f"KDJ触发SELL信号 - 前J:{prev_j:.2f}, 前K:{prev_k:.2f}, 前D:{prev_d:.2f}, 当前J:{last_j:.2f}, 当前K:{last_k:.2f}, 当前D:{last_d:.2f}"
        )
        return "SELL"
    ...
    # 新增SELL条件：prev_j小于prev_k和prev_d，且last_j<last_k和last_d，且last_j<prev_j且last_j>5
        ...
        g.last_kdj_signal = "SELL"
        log.info(
            f"KDJ触发新增SELL信号 - 前J:{prev_j:.2f}, 前K:{prev_k:.2f}, 前D:{prev_d:.2f}, 当前J:{last_j:.2f}, 当前K:{last_k:.2f}, 当前D:{last_d:.2f}"
        )
        return "SELL"
    # 2. BUY条件：当前J < 0
        ...
        log.info(
            f"KDJ触发BUY信号 - 当前J:{last_j:.2f}, 当前K:{last_k:.2f}, 当前D:{last_d:.2f}"
        )
        return "BUY"
    ...
    if hasattr(g, "trading_signal") and g.trading_signal == False:
        ...
        for stock in positions:
            if stock in getattr(g, "stock_pool", []) or stock in g.target_list:
                ...
                timing_signal = get_timing_signal_with_now_price(
                    context, stock, current_price=current_price
                )
                ...
                if timing_signal not in ["KEEP", "BUY"]:
                    ...
                    print(
                        f"尾盘卖出信号：{stock} timing_signal={timing_signal}，执行清仓"
                    )
                    # 设置全局参数，停止买入etf两个交易日
                    g.etf_buy_block_days = 3
                else:
                    print(
                        f"尾盘信号检查：{stock} timing_signal={timing_signal}，不清仓"
                    )
...
def get_current_price_robust(stock, context):
    ...
    try:
        current_data = get_current_data()
        if (
            stock in current_data
            and hasattr(current_data[stock], "last_price")
    ...
    try:
        ...
        if (
            not recent_prices.empty
            and stock in recent_prices.columns
            and len(recent_prices[stock]) > 0
        ):
    ...
    try:
        ...
        if (
            not daily_prices.empty
            and stock in daily_prices.columns
            and len(daily_prices[stock]) > 0
        ):
